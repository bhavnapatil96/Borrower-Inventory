...
import {ProductList,ProductUnApproved} from '../../action/product_action/index'
...
let flag='asc';
class Product_Grid extends Component{
    constructor(){
        super();
        this.state={
            product_list:[],
    ...
    toggleApproved=(e,pid,s)=>{
            e.preventDefault();
            let obj={
                productId:pid,
                approved:s,
            }
            this.props.ProductUnApproved(obj);
    }
    mypage=(no)=>{
    ...
    search=(e)=> {
       ...
       this.setState({isSearch:true,searchArr:[]})
        var temp=[]
                                ...
                                <div class="table-responsive">
                                    <table id="myTable" className="table table-hover display table" width="100%" >
                                        ...
                                        <tr>
                                            ...
                                            <th id="userId" onClick={this.sort}>Owner</th>
                                                ...
                                                this.state.searchArr.map((pr,i)=>{
                                                    ...
                                                    return(
                                                        <tr>
                                                            ...
                                                            {
                                                                (pr.isApprove===0)?
                                                                    <td><button className="btn btn-success" onClick={(e)=>{this.toggleApproved(e,pid,1)}}>{'Approved'}</button></td>
                                                                    :
                                                                    <td><button className="btn btn-danger" onClick={(e)=>{this.toggleApproved(e,pid,0)}}>{'Not Approved'}</button></td>
                                                            }
                                                        </tr>
                                                    )
                                                })
...
function mapStateToProps(state) {
    return {
        product_list:state.all_items,
        user:state.user
    }
}
function matchDispatchToProps(dispatch) {
    return bindActionCreators({ProductList,UserList,ProductUnApproved},dispatch)
}
export default connect(mapStateToProps,matchDispatchToProps)(Product_Grid);
